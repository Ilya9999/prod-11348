<?php
/**
 * Itcompany functions and definitions.
 */
add_filter( 'kava-theme/assets-depends/styles', 'itcompany_styles_depends' );
add_action( 'jet-theme-core/register-config', 'itcompany_core_config' );
add_action( 'init', 'itcompany_plugins_wizard_config', 9 );
add_action( 'init', 'itcompany_data_importer_config', 9 );
add_action( 'tgmpa_register', 'itcompany_register_required_plugins' );
/**
 * Enqueue styles.
 */
function itcompany_styles_depends( $deps ) {
 ...
 */
function itcompany_core_config( $manager ) {
	$manager->register_config(
		array(
			'dashboard_page_name' => esc_html__( 'Itcompany', 'itcompany' ),
 ...
 * @return [type] [description]
 */
function itcompany_plugins_wizard_config() {
	if ( ! is_admin() ) {
		return;
	}
		...
		'skins'   => array(
			...
			'advanced' => array(
				'default' => array(
					...
					'name'  => esc_html__( 'Itcompany', 'itcompany' ),
					),
				),
			),
		'texts'   => array(
			'theme-name' => esc_html__( 'Itcompany', 'itcompany' ),
		)
	) );
}
/**
 ...
 * @return [type] [description]
 */
function itcompany_data_importer_config() {
	if ( ! is_admin() ) {
		return;
	}
	...
	jet_data_importer_register_config( array(
		...
		'advanced_import' => array(
			'default' => array(
				'label'    => esc_html__( 'Itcompany', 'itcompany' ),
...
*/
		...
		'slider' => array(
			'path' => 'https://raw.githubusercontent.com/ZemezLab/kava-itcompany/master/slides.json',
		),
	) );
}
/**
